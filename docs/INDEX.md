...
**No specification content lives here**, so this file cannot contradict its sources. It only points.
...
| **Releasing a version** | `ARCHITECTURE 6.3` | `ARCHITECTURE 6.1`, `1.2` | both |
...
| How do I publish? | `ARCHITECTURE 6.3` |
...
| **Dead links in `design-framework/`** | Four files linked `documentation/design-framework-uiux-promax-integration-plan.md`, which has never existed. | `ARCHITECTURE 7.1` |
| **Version in three places** | `package.json` said `0.2.0`, `package-lock.json` said `0.1.2`, and no tag existed — so nothing was published. | `ARCHITECTURE 6.3` |
